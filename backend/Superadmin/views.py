...
def get_admin(request):
    pics=Superadmin.objects.get(id=4)
...
def get_statsdata(request):
    ...
    if type=='total_consumption':
        ...
        for meter in meters.data:
            ...
            print(str(reading.reading_value))
...
def memory(request):
    ...
    return Response([{
        ...
        "database":"Supervisor Table",
        "bytesusage":total_size_supervisor},{
        "database":"Meter Table",
        "bytesusage":meters_size},{
        "database":"Meter Readings Table",
...
def supervisors(request):
    if isAuthenticated(request)==False:
        return Response(data={'message':" please login again"},status=403)
    operation = request.GET.get('operation')
    if operation =='get':
...
def delete_meter(request):
    ...
    try:
        meters = Meters.objects.get(id=id)
